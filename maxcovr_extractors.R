...
augment_user <- function(facilities_selected,
  ...
  if (!is.null(existing_facilities)) {
    ...
    all_facilities <- facilities_selected[ , c("lat", "long")] %>%
      dplyr::mutate(type = "selected")
    }
  
  # now return the distances etc
  augmented_users <- maxcovr::nearest(all_facilities,
                             existing_users)
  # augmented_users <- nearest_motus(all_facilities,
  #                                     existing_users)
