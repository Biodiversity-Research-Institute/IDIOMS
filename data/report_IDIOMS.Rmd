...
cat(paste0("IDIOMS - Informing the Design and Implementation of Offshore Motus Systems version ", params$IDIOMS_version,"\nwas run for ", params$option, "\n", 
...
selected_stns <- params$output_df %>% 
      data.frame() %>% 
      select(-c(geometry)) %>% 
      rename("Flight Height"="flt_ht", "Station ID"="station_ID", "Long."="long", "Lat."="lat","Computed antenna angle"="theta")

kbl(selected_stns, booktabs=T, caption = "Stations and selected antenna angles from IDIOMS run.") %>%
  kable_styling(position="center", full_width=F, latex_options = c("HOLD_position", "striped")) %>%
    row_spec(0,bold=T) %>% 
...
study_covg <- params$study_covg %>% 
  mutate(max_r=round(max_r, 0), 
         antenna_coverage_overlap=round(antenna_coverage_overlap, 2), 
         antenna_coverage_overlap=round(antenna_coverage_overlap, 2)) %>% 
      rename("Flight height (m)"="flt_ht", "Max detection distance (m)"="max_r", "Prop. of study area covered"="study_area_covered", "Prop. of adjacent antenna overlap"="antenna_coverage_overlap")
...
if (!is.na(params$simulated_tracks)){
  simulated_tracks <- params$simulated_tracks %>% 
    rename("Species group"="taxa",
...
MOTUS_receiver_antennas_active_sf_merc_near <- MOTUS_receiver_antennas_active_sf_merc[st_intersects(MOTUS_receiver_antennas_active_sf_merc,
                                                                                                    st_union(params$output_df) %>% st_buffer(40000), sparse = F) , ]
...
for (flt_ht in sort(unique(params$output_df$flt_ht))){
  ...
  shut_up(
    print(basemaps::basemap_ggplot(ext = antenna_bbox_buff_merc_lg, map_service = "esri", map_type = "world_ocean_base") +
                  ...
                  # geom_sf(data=ant_at_ht_grid_merc, col="navy", fill=NA) +
...
if (!is.na(params$seabird_sims)){
  ...
  for (flt_ht in sort(unique(params$output_df$flt_ht))){
    ...
    shut_up(print(basemaps::basemap_ggplot(ext = antenna_bbox_buff_merc_lg, map_service = "esri", map_type = "world_ocean_base") +
                    ...
                    # geom_sf(data=ant_at_ht_grid_merc, col="navy", fill=NA) +
                    geom_sf(data=study_boundary_merc, aes(col="red"), fill=NA) +
                    geom_sf(data=params$seabird_sims, aes(col="lawngreen")) +
                    scale_color_identity(labels = c(gray40 = "BOEM wind leases", gray80="BOEM planning areas",
                                                    darkorange1 = "Motus stations", navy="Antenna beam pattern", 
                                                    red="Study area boundary", lawngreen="Seabird tracks"), guide = "legend") +
                    coord_sf(xlim = c(antenna_bbox_buff_merc_med[["xmin"]], antenna_bbox_buff_merc_med[["xmax"]]), 
                             ylim = c(antenna_bbox_buff_merc_med[["ymin"]], antenna_bbox_buff_merc_med[["ymax"]])) +
...
if (!is.na(params$shorebird_sims)){
  #buffer bounding box by x km to get view of area with antenna array
  antenna_bbox_buff_merc_lg <- st_union(params$output_df) %>% st_buffer(60000) %>% st_bbox() 
  antenna_bbox_buff_merc_med <- st_union(params$output_df) %>% st_buffer(40000) %>% st_bbox() 
  ...
  for (flt_ht in sort(unique(params$output_df$flt_ht))){
    ...
    shut_up(print(basemaps::basemap_ggplot(ext = antenna_bbox_buff_merc_lg, map_service = "esri", map_type = "world_ocean_base") +
                    ...
                    # geom_sf(data=ant_at_ht_grid_merc, col="navy", fill=NA) +
                    geom_sf(data=study_boundary_merc, aes(col="red"), fill=NA) +
                    geom_sf(data=params$shorebird_sims, aes(col="lawngreen")) +
                    scale_color_identity(labels = c(gray40 = "BOEM wind leases", gray80="BOEM planning areas",
                                                    darkorange1 = "Motus stations", navy="Antenna beam pattern", 
                                                    red="Study area boundary", lawngreen="Shorebird tracks"), guide = "legend") +
                    coord_sf(xlim = c(antenna_bbox_buff_merc_med[["xmin"]], antenna_bbox_buff_merc_med[["xmax"]]), 
                             ylim = c(antenna_bbox_buff_merc_med[["ymin"]], antenna_bbox_buff_merc_med[["ymax"]])) +
