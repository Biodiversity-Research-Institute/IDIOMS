...
# 6 Sep 24 - ATG sigfig issue leading to polys not closed error
...
createOmni <- function(lat, lng) { 
  ...
  # # 06 Sep 22 - does not work for lat 50.8709 long -0.0837 possibly due to date line issue
  ...
  poly_sf <-  st_sfc(st_point(c(lng,lat)), crs = 4326) %>% 
    st_transform(3857) %>% 
    st_buffer(dist = 1000) %>% 
    st_transform(4326) %>% 
...
createFake <- function(lat, lng, bearing) {
  ...
  for (i in seq(0, TAU, inc)) {
    ...
    while(lat > 90){
      lat <- lat - 180}
    # 16 Sep 24 - ATG sigfig issue leading to polys not closed error where first and last points
    # not the same
    path[[j]] <- list(lng=round(lng, 6), lat=round(lat, 6))
  }
  #create a polygon from the paths in R
  path_pts <- rbindlist(path)
  result <- tryCatch({
...
MOTUS_antenna_geometry <- function(ID, ant_type, lat, lng, bearing = NULL) {
  ...
  suppressWarnings(if(class(ant_geom)[[2]]=="sf" | class(ant_geom)[[2]]=="data.frame"){
...
get_motus_receiver_antenna_data <- function(){
  ...
  MOTUS_receiver_antennas <- dplyr::full_join(MOTUS_receiver_deploy, MOTUS_antenna_deploy, multiple = "all") 
